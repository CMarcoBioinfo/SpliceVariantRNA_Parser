...
# Colonnes par catégorie (héritées de ton ancien sample_window)
COLUMNS_BY_SOURCE = {
...
def analyze_patient(run_path, group_zip, sample, session_tmp):
    """
    Version normale :
    - lit recap.xlsx via parse_recap
    - convertit les lignes en événements via row_to_event
    - regroupe par catégorie
    - prépare les chemins QC et Sashimi
    - renvoie un PatientAnalysisResult propre
    """

    # ------------------------------
    # 1) Lire recap.xlsx du patient
    # ------------------------------
    rows = parse_recap(run_path, group_zip, sample)
    events = [row_to_event(r, sample) for r in rows]

    # ------------------------------
    # 2) Normaliser les sources
    # ------------------------------
    for ev in events:
        if "Source" in ev:
    ...
    for ev in events:
        ...
        if src in events_by_cat:
            ...
            # Catégorie inconnue → on peut la logguer plus tard
            pass
